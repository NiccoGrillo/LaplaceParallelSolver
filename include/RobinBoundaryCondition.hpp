...
public:
    ...
    void setBoundaryConditions(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& local_U, int rank, int size, int n, double h) override {
        applyBoundaryConditionsAtIteration(local_U, rank, size, n, h);
    }
    ...
    void applyBoundaryConditionsAtIteration(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& local_U, int rank, int size, int n, double h) override {
        ...
        if (rank == 0) {
            for (int j = 0; j < local_U.cols(); ++j) {
                double y = j * h;
                local_U(0, j) = (g_func(0, y)*h + beta * local_U(1, j)) / (alpha + beta);
            }
        }
        if (rank == size - 1) {
            for (int j = 0; j < local_U.cols(); ++j) {
                double y = j * h;
                local_U(local_U.rows() - 1, j) = (g_func(1, y)*h + beta * local_U(local_U.rows() - 2, j)) / (alpha + beta);
            }
        }
        for (int i = 0; i < local_U.rows(); ++i) {
            double x = (rank * local_U.rows() + i) * h;
            local_U(i, 0) = (g_func(x, 0)*h + beta * local_U(i, 1)) / (alpha + beta);
            local_U(i, local_U.cols() - 1) = (g_func(x, 1)*h + beta * local_U(i, local_U.cols() - 2)) / (alpha + beta);
        }
    }
